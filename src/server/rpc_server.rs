...
//!
//! This module contains the core [`RpcServer`] type which handles incoming RPC
//! requests over a transport, dispatches them to registered handlers, and sends
//! responses back.
//!
//! # Architecture
//!
//! The server subscribes to `requests/{node_id}` on the configured transport,
//! processes incoming request envelopes, dispatches to typed handlers, and
//! publishes responses to the reply-to address specified in each request.
//!
//! Handler registration is thread-safe (protected by a mutex) and handlers
//! are invoked concurrently as requests arrive.
//!
//! # Usage Patterns
//!
//! Two main patterns are supported:
//!
//! - **Blocking**: Use [`RpcServer::run()`] in your main task. The server runs
//!   until [`RpcServer::shutdown()`] is called from another task.
//!
//! - **Background**: Use [`RpcServer::spawn()`] to run the server in a background
//!   task, allowing the main task to do other work. Remember to call
//!   [`RpcServer::shutdown()`] before closing the transport.
